...
public class ArticleController {
    ...
    public SmallTalkResponse<Article> getArticle(@PathVariable String id) throws NoArticleFoundException {
        return new SmallTalkResponse<>(service.getArticleById(id));
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public SmallTalkResponse<Article> addArticle(@RequestBody Article article) throws ArticleAlreadyExistsException {
        return new SmallTalkResponse<>(service.addArticle(article));
    }

    @PatchMapping("/publish/{id}")
    @ResponseStatus(HttpStatus.OK)
    public SmallTalkResponse<Article> publishArticle(@PathVariable String id) throws NoArticleFoundException {
        return new SmallTalkResponse<>(service.publishArticle(id));
    }

    @PatchMapping()
    @ResponseStatus(HttpStatus.OK)
    public SmallTalkResponse<Article> updateArticle(@RequestBody Article article) throws NoArticleFoundException {
        return new SmallTalkResponse<>(service.updateArticle(article));
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteArticle(@PathVariable String id) throws NoArticleFoundException {
